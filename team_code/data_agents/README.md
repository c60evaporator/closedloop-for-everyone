...
### shasou-recorderとの併用について

データ収集用ツールキット[shasou-recorder]()と`ROS2DataAgentShasou`クラス（前述の`GeneralizedROS2DataAgent`を継承したクラス）を組み合わせることで、shasouエコシステムを用いた性能評価に基づくデータキュレーションを実現できます。


上で記載したTopicのうち、`/tf`（`<namespace>/gt/ego_odom`と重複するため）と`<namespace>/gt/object_attributes`（有用性が低いため）は、受信側ユニットであるshasou-recorderの記録スコープから除外しています
