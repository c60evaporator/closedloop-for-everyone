...
- [`ROS2DataAgentShasou`]()：収集したセンサデータを**ROS2 Topicで出力し、shasou-recoderで受信・管理**するためのエージェントベースクラス
...
|`<namespace>/gt/object_attributes`|std_msgs/String|-（JSON内に記載）|-|全アクターの速度・LiDAR点数（`gt/objects`の付随情報をJSONで格納）|
...
##### `<namespace>/gt/objects`と`<namespace>/gt/object_attributes`（instance IDと付随情報）

`vision_msgs/Detection3DArray`は速度やLiDAR点数を格納するフィールドを持たないため、これらは別トピック`<namespace>/gt/object_attributes`（`std_msgs/String`）にJSON形式で出力し、**instance IDで`gt/objects`と紐付ける**構成としています。

|情報|格納先|内容|
|---|---|---|
|instance ID|`gt/objects`の`Detection3D.id`（string）|`<route_index>_<CARLA actor id>`。同一アクターに対しシーン内の全フレームで不変なので、nuScenesの`instance_token`の生成元として使用できる|
|物体クラス|`gt/objects`の`results[0].hypothesis.class_id`|CARLAのblueprint type_id（例：`vehicle.tesla.model3`）をそのまま格納。`score`は常に1.0|
|前進速度 [m/s]|`gt/object_attributes`の`speed_mps`|-|
|bbox内のLiDAR点数|`gt/object_attributes`の`num_lidar_pts`|nuScenes `sample_annotation.num_lidar_pts`相当|

`gt/object_attributes`は1フレームにつき以下のような1つのJSON文書を出力します。

```json
{"stamp":{"sec":12,"nanosec":500000000},"frame_id":"map","objects":[{"instance_id":"3_42","speed_mps":0.5,"num_lidar_pts":8}]}
```

- **紐付け方法**：`std_msgs/String`はheaderを持たないためJSON内に`stamp`を埋め込んでいます。受信側は`stamp`が一致する`gt/objects`メッセージを特定し、`objects[].instance_id`と`Detection3D.id`を突き合わせてください。両トピックは同一フレーム・同一stampで同時にpublishされます
- CARLAのactor IDはエピソード（ルート）ごとに振り直されるため、instance IDにはルート番号を接頭辞として付与しています。**シーンを跨いだ一意性は保証されません**
- `num_lidar_pts`は、直近に出力が完了した1台目のLiDARの360度点群を用いて算出します。**LiDARを搭載していないセンサ構成の場合や、最初のスイープが完成する前のフレームでは、`num_lidar_pts`キー自体が出力されません**（誤解を招く`0`を出さないため）。同様に速度が不明なオブジェクトでは`speed_mps`キーが出力されません
- **nuScenesの`visibility`は出力していません**。nuScenesのvisibilityは6カメラ画像における可視画素の割合であり、LiDAR点数からの代理指標では定義が乖離するためです。必要な場合は`num_lidar_pts`から受信側で算出してください（`tools/nuscenes/convert_to_nuscenes.py`の`visibility_for_points`がファイル保存側で使用しているbinning閾値の実装例です）

## ROS2DataAgentShasou

収集したセンサデータを**ROS2 Topicで出力し、データ収集用ツールキット[shasou-recorder]()で受信・管理**するためのエージェントのベースクラスです。`GeneralizedROS2DataAgent`を継承しており、センサデータのTopic出力に加えて、**1ルート=1収録（=nuScenesの1 log）**の対応でshasou-recorderに収録の開始・終了を指示します。これにより、ルート切り替え時の無音区間がbagに混入せず、shasouエコシステムを用いた性能評価に基づくデータキュレーションを実現できます。

なお実行には、`shasou_msgs`パッケージがインポート可能で、かつshasou-recorderが起動してサービスを提供している必要があります（[詳細は後述](#shasou-recorderとの連携)）。

### 自作エージェント作成方法

以下の手順で自作エージェントを作成します

- 自作クラスファイルの作成：`ROS2DataAgentShasou`を継承した自作クラスを`team_code/data_agents`フォルダ内に作成する
- クラス定数の記述：自作クラスの冒頭にクラス定数を適切に記述する
- センサ構成の記述：自作クラスの`_sensors`メソッド内にセンサ構成を記述する

#### 自作クラスファイルの作成

`ROS2DataAgentShasou`を継承した自作クラスを`team_code/data_agents`フォルダ内に作成し、`TOPIC_NAMESPACE`等の各種クラス定数（[詳細は後述]()）と、`_sensors`メソッド内のセンサ構成（[GeneralizedDataAgentの記載方法]()と同様）を記述します。なお、出力されるTopicのフォーマットはROS2の定義に従うため、フォーマットを指定するためのクラス定数の記述は不要です。

例えばnuScenes形式でデータをTopic出力するエージェント`ros2_data_agent_shasou_nuscenes.py`は以下のように作成できます。

```python
from ros2_data_agent_shasou import ROS2DataAgentShasou

def get_entry_point():
    return 'ROS2DataAgentShasouNuScenes'

class ROS2DataAgentShasouNuScenes(ROS2DataAgentShasou):
    """
    Child of ROS2DataAgentShasou with a nuScenes-style 6 camera + LiDAR + GNSS rig.
    """
...
実行時は、Dockerコンテナ内で**shasou-recorderを起動した状態で**、以下コマンドを打つことで作成したエージェントを用いたデータ収集・収録が始まります（recorderが起動していない場合の挙動は[後述](#前提条件とサービス利用不可時の挙動)の`RECORDER_REQUIRED`で切り替えます）。
...
例えば上のROS2DataAgentShasouNuScenesを使用する場合、以下を実行します。

```bash
TEAM_AGENT=team_code/data_agents/ros2_data_agent_shasou_nuscenes.py \
bash tools/collect_dataset_multi.sh ${CARLA_GARAGE_ROOT}/data
```

#### クラス定数の記述

自作クラス内で以下のクラス定数を定義します。`ROS2DataAgentShasou`は`GeneralizedROS2DataAgent`を継承しているため、`REAR_AXLE_TO_CENTER`と`TOPIC_NAMESPACE`は同様に必須です。加えて、shasou-recorderとの連携動作を制御する定数を任意で指定できます。

|定数名|必須|型|内容|
|---|---|---|---|
|**`REAR_AXLE_TO_CENTER`**|☑️|float|車両中心から後車軸までの距離（単位メートル）。Leaderboardのデフォルト車両Lincoln MKZを使用する場合は`1.42`を指定|
|**`TOPIC_NAMESPACE`**|☑️|str|出力されるセンサデータ等のトピックの名前空間。例えば`TOPIC_NAMESPACE="/shasou"`とした場合、idが"lidar_top"のLiDARのトピック名は`/shasou/lidar_top/points`となる|
|`RECORDER_REQUIRED`||bool|recorderサービス（または`shasou_msgs`）が利用できない場合の挙動。`True`（デフォルト）では`setup()`で例外を送出して停止し、`False`では警告を出して収録なしで走行を継続する|
|`RECORDER_WAIT_TIMEOUT_SEC`||float|`setup()`でrecorderサービスの出現を待つ最大時間 [秒]（デフォルト`10.0`）|
|`RECORDER_RESPONSE_TIMEOUT_SEC`||float|各サービス呼び出しの応答を待つ最大時間 [秒]（デフォルト`30.0`）。StopRecordingはbagのフラッシュとmanifest書き出しを含むため長めに設定している|

なお、呼び出すサービス名は`START_RECORDING_SERVICE`／`STOP_RECORDING_SERVICE`定数（既定で`/shasou/recorder/start_recording`・`/shasou/recorder/stop_recording`）で保持していますが、shasou-recorderの規約に固定されているため通常は上書き不要です。
...
`ROS2DataAgentShasou`は、ルートの走行ライフサイクルに合わせてshasou-recorderのROS2サービスを呼び出します。

|タイミング|呼び出すサービス|内容|
|---|---|---|
|最初の`run_step()`（PDM-Liteの初期化と天候の確定後）|`/shasou/recorder/start_recording`（`shasou_msgs/srv/StartRecording`）|収録を開始する。ルートにつき1回だけ呼ばれる|
|`destroy()`（ルート終了時）|`/shasou/recorder/stop_recording`（`shasou_msgs/srv/StopRecording`）|収録を終了する。応答（bagクローズとmanifest書き出しの完了）を待ってからノードを破棄する|

収録は開始～終了の間だけ行われるため、**ルート＝収録＝nuScenesの1 log**が正確に対応し、ルート切り替え時の無音区間がbagに混入しません。

`StartRecording`では、recorder自身が知り得ない以下の情報をCARLAのworld stateから取得して送信します（platform／vehicle／calib_id等はrecorder側の設定が保持するため送りません）。取得できない場合は空文字にフォールバックして走行を継続します。

|フィールド|送信される値|
|---|---|
|`source`|`"carla"`固定|
|`location`|CARLAのマップ名（例：`"Town12"`）。`self._world.get_map().name`はパス形式で返るため末尾のマップ名のみを取り出す。`manifest.location`＝nuScenesの`log.location`になる|
|`route_id`|ルート番号（`route_index`）。`tags["route_id"]`に入る|
|`scenario`|シナリオ名（`scenario_name`）。`tags["scenario"]`に入る|
|`weather`|収録開始時点の天候を分類したBench2Driveのプリセット名（例：`"ClearNoon"`）。`manifest.weather`になる|

- `weather`は、CARLAの現在の天候（`carla.WeatherParameters`）をBench2Driveの23種類の天候プリセットへ最近傍分類した名前です。時間帯（sun_altitude_angle）は視覚的に支配的なため距離計算で重めに扱っています。走行中に天候は変化しない前提のため、収録開始時点の分類が走行全体で有効です
- `StopRecording`の`completed`フラグには、Leaderboardの評価結果（ルートが完走したか）が反映されます。異常終了（衝突・タイムアウト等）の場合は`completed=false`となり、`reason`に理由が入ります。studio側で異常終了ドライブを除外するフィルタに利用できます（評価結果を取得できない一部の実行経路では`completed=true`として扱われます）

#### 前提条件とサービス利用不可時の挙動

本エージェントの実行には、以下が必要です。

- `shasou_msgs`パッケージがエージェントのインタプリタからインポート可能であること（ビルド済みの`shasou-msgs` colconワークスペースをROS2 Humble上でsourceしておく）
- shasou-recorderが起動し、`/shasou/recorder/start_recording`および`/shasou/recorder/stop_recording`サービスを提供していること

これらが満たされない場合の挙動は、クラス定数`RECORDER_REQUIRED`で切り替えられます。

- `RECORDER_REQUIRED=True`（デフォルト）：`setup()`が明確なエラーメッセージで例外を送出して停止します（収録するつもりで走ったのに録れていない、という事態を防ぐため）
- `RECORDER_REQUIRED=False`：警告を出して収録なしで走行を継続します（recorderなしで動作確認したい場合。収録が不要なら、shasou非依存の`ROS2DataAgentNuScenes`を使う選択肢もあります）

### 出力するTopicの形式

- 基本的には[GeneralizedROS2DataAgentと同じ構成のTopic]()を出力します
- 出力するTopicのうち、`/tf`（`<namespace>/gt/ego_odom`と重複するため）と`<namespace>/gt/object_attributes`（有用性が低いため）は、受信側ユニットであるshasou-recorderの記録スコープから除外しています
