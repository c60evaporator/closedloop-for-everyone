...
##### `<namespace>/gt/objects`の補足（instance IDと付随情報）

`vision_msgs/Detection3DArray`は速度やvisibility等を格納するフィールドを持たないため、以下のように情報を格納しています。

|情報|格納先|内容|
|---|---|---|
|instance ID|`Detection3D.id`（string）|`<route_index>_<CARLA actor id>`。同一アクターに対しシーン内の全フレームで不変なので、nuScenesの`instance_token`の生成元として使用できる|
|物体クラス|`results[0].hypothesis.class_id`|CARLAのblueprint type_id（例：`vehicle.tesla.model3`）をそのまま格納。`score`は常に1.0|
|前進速度 [m/s]|`class_id='speed_mps'`のhypothesisの`score`|-|
|bbox内のLiDAR点数|`class_id='num_lidar_pts'`のhypothesisの`score`|nuScenes `sample_annotation.num_lidar_pts`相当|
|visibility|`class_id='visibility'`のhypothesisの`score`|1〜4。nuScenesの`visibility_token`（v0-40 / v40-60 / v60-80 / v80-100）に対応|

- **`results[0]`以外はメタデータ枠**なので、受信側は必ず`class_id`でフィルタしてください（順序や個数を前提にしないでください）
- CARLAのactor IDはエピソード（ルート）ごとに振り直されるため、`Detection3D.id`にはルート番号を接頭辞として付与しています。**シーンを跨いだ一意性は保証されません**
- `num_lidar_pts`は、直近に出力が完了した1台目のLiDARの360度点群を用いて算出します。**LiDARを搭載していないセンサ構成の場合や、最初のスイープが完成する前のフレームでは、`num_lidar_pts`と`visibility`のhypothesisは出力されません**
- **`visibility`はnuScenesの厳密な定義とは異なります**。nuScenesのvisibilityは6カメラ画像における可視画素の割合ですが、ここではLiDAR点数からの代理指標として算出しています（binningの閾値は`tools/nuscenes/convert_to_nuscenes.py`の`visibility_for_points`と一致）。厳密な可視率を得るにはインスタンスセグメンテーションカメラの追加が必要です
